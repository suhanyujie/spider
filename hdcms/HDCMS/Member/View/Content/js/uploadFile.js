...
function files(obj) {
    ...
    地址: <input name="'+name+'[path][]" value="'+path+'" style="width:30%" readonly="" type="text"> &nbsp;&nbsp;\
    描述: <input name="'+name+'[alt][]" style="width:30%;" value="" type="text">\
        &nbsp;&nbsp;\
...
function thumb(obj) {
    ...
    $(parent.document).find('input[name="'+name+'"]').parent().find('*').show();
    parent.hd_modal_close();
}
/**
 * 获取站内文件
...
function webFile(type, page) {
    $.get(CONTROLLER + '&a=webFile', {mid:mid,type: type, page: page}, function (html) {
        $("#webFile").html(html);
    })
}
$(function () {
...
function noUse(type, page) {
    $.get(CONTROLLER + '&a=noUse', {mid:mid,type: type, page: page}, function (html) {
        $("#noUse").html(html);
    })
}
$(function () {
