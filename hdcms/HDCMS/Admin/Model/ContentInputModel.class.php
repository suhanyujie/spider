...
class ContentInputModel {
	...
	private function thumb( $fieldInfo, $value ) {
		...
		if ( isset( $_POST['auto_thumb'] ) && $_POST['auto_thumb'] == 1 && isset( $_POST['content'] ) && extension_exists( 'curl' ) ) {
			if ( preg_match_all( '/\s+(src)=([\'"]?)([^\'">]+\.(jpg|jpeg|png|gif))\2/is',
				$_POST['content'], $matchData, PREG_PATTERN_ORDER ) ) {
	...
	private function content( $fieldInfo, $value ) {
		...
		//下载内容图片
		if ( isset( $_POST['down_remote_pic'] )
		     && $_POST['down_remote_pic'] == 1
		     && extension_exists( 'curl' )
		) {
			$Attachment = new Attachment();
			$value      = $Attachment->download( $value,
				array( 'jpg', 'gif', 'jpeg', 'png' ), null, $this->mid );
		}
